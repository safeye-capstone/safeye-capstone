...
def calculate_temporal_support(
    ...
    for start_index in range(
        len(detection_flags) - actual_window_size + 1
    ):
        window = detection_flags[
            start_index:start_index + actual_window_size
        ]

        support = sum(window) / len(window)
        best_support = max(best_support, support)

    return round(best_support, 3)
...
def aggregate_frame_results(
    ...
    frame_names = [
        frame_result.get("frame", f"frame_{index}")
        for index, frame_result in enumerate(
            frame_results,
            start=1,
        )
    ]

    total_frame_count = len(frame_names)
    ...
    for frame_result in frame_results:
        frame_name = frame_result.get("frame", "unknown")
        ...
        for hazard in hazards:
            detected = hazard.get("detected", False)
            ...
            for risk_type in risk_types:
                ...
                if risk_type not in frame_seen_risks:
                    buckets[risk_type]["frames"].append(frame_name)
                ...
                if evidence:
                    buckets[risk_type]["evidence"].append(evidence)
                ...
                previous_score = buckets[
                    risk_type
                ]["confidence_by_frame"].get(frame_name)

                if (
                    previous_score is None
                    or confidence_score > previous_score
                ):
                    buckets[
                        risk_type
                    ]["confidence_by_frame"][frame_name] = confidence_score
    ...
    for risk_type, data in buckets.items():
        ...
        detection_count = len(evidence_frames)

        required_count = MIN_DETECTION_COUNT.get(
            risk_type,
            min_detection_count,
        )
        ...
        meets_min_detection_count = (
            detection_count >= required_count
        )
        ...
        if total_frame_count:
            detection_ratio = detection_count / total_frame_count
        else:
            detection_ratio = 0.0
        ...
        detection_flags = [
            1 if frame_name in evidence_frame_set else 0
        ...
        max_consecutive_count = calculate_max_consecutive_count(
            detection_flags
        )
